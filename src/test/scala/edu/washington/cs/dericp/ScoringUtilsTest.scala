...
import edu.washington.cs.dericp.ScoringUtils.Scores
...
class ScoringUtilsTest {
  ...
  def testPrecision(): Unit = {
    val p = ScoringUtils.precision(4, 4)
    assertEquals(0.5, p, 0)
  }

  @Test
  def testRecall(): Unit = {
    val r = ScoringUtils.recall(4, 2)
    assertEquals(4.0/6.0, r, 0)
  }

  @Test
  def testF1ScoreSimple(): Unit = {
    val f1 = ScoringUtils.f1Score(0.5, 0.5)
  ...
  def testF1ScoreComplex(): Unit = {
    val f1 = ScoringUtils.f1Score(4, 4, 4)
    val expected = 2 * (0.5 * 0.5) / (0.5 + 0.5)
    assertEquals(expected, f1, 0)
  }
  ...
  def testAvgPrec(): Unit = {
    ...
    val AP = ScoringUtils.avgPrec(avgPrecResults, avgPrecCorrectResults, falseNeg)
  ...
  def testAvgPrecShortResults(): Unit = {
    ...
    //println("false neg: " + falseNeg)
    val AP = ScoringUtils.avgPrec(avgPrecShortResults, avgPrecCorrectResults, falseNeg)
  ...
  def testMeanAvgPrec(): Unit = {
    ...
    val MAP = ScoringUtils.meanAvgPrec(scoresList)
  ...
  def testGetScoresFromResults(): Unit = {
    val scores = ScoringUtils.getScoresFromResults(results, correctResults)
    val p = 0.5
    val r = 0.5
